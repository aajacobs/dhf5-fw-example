...
function main(content, options) {
  //grab the doc id/uri
  let id = content.uri;
  xdmp.log(`content: ${JSON.stringify(content)}`);
  ...
  const config = fn.head(fn.collection("fwconfig")).root.envelope.instance;

  const process = (config, line) => {
    let obj = {};

    for (const key of Object.keys(config)) {
      const { start_pos, end_pos, type } = config[key];
      xdmp.log(`type: ${type}`);
      let str = line
        .substring(start_pos, end_pos)
        .split(" ")
        .join("");
      obj[key] = type == "int" ? fn.number(str) : xs.string(str);
    }
    return obj;
  };
  ...
  let instance = process(config, doc.toObject().row);
